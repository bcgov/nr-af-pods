/*
  ...
  -------------------
  version: 1.0.0
  name: quartech_application-form-script
  type: JavaScript (JS)
  description: This is the Web Resource called "quartech_application-form-script"
  */
...
(function () {
  ...
  this.showSections = async function (visibleSections) {
    ...
    sectionsToShow.forEach((sectionName) => {
      if (sectionName != '') {
        ...
        if (tabControl) {
          Xrm.Page.ui.tabs
            .get('tab_Deliverables_Budget')
            .sections.get('tab_Deliverables_Budget_section_Coding')
            .setVisible(false);
  ...
  this.initProgramLookupOnChange = async function (formContext) {
    formContext
      .getControl('quartech_program')
      .getAttribute()
      .addOnChange(PODS.handleProgramLookup_OnChange);
  };
  ...
  this.customizeFormByProgram = async function (formContext) {
    const programConfigData = await PODS.getProgramStreamConfigData(
      formContext
    );

    PODS.customizeFormByConfiguredProgramBusinessLogics(
      formContext,
      programConfigData
    );
    ...
    PODS.customizeFormByJsonConfig(
      formContext,
      programConfigData.quartech_staffportalapplicationformjsonconfig
    );
  };

  this.customizeFormByConfiguredProgramBusinessLogics = async function (
    formContext,
    programBusinessLogics
  ) {
    const isBusinessLogicsNotEnabled =
      !programBusinessLogics?.quartech_businesslogicsenabled;
    if (isBusinessLogicsNotEnabled) return;

    const podsConfigData = JSON.parse(
      programBusinessLogics.quartech_StaffPortalConfig?.quartech_configdata
    );

    if (!podsConfigData) return;

    // Show Hide for Application Information's Business Summary section
    PODS.showHideFieldsBySelectedProgram(
      formContext,
      podsConfigData.BusinessLogics?.ApplicationInformation
        ?.BusinessSummary_FieldNamesValues_Map,
    ...
    PODS.showHideFieldsBySelectedProgram(
      formContext,
      podsConfigData.BusinessLogics?.ApplicationInformation
        ?.NaicsCode_FieldNamesValues_Map,
    ...
    PODS.showHideFieldsBySelectedProgram(
      formContext,
      podsConfigData.BusinessLogics?.ApplicationInformation
        ?.DocumentSubmission_FieldNamesValues_Map,
    ...
    PODS.showHideFieldsBySelectedProgram(
      formContext,
      podsConfigData.BusinessLogics?.ApplicationInformation
        ?.ProgramEligibility_FieldNamesValues_Map,
    ...
    PODS.showHideFieldsBySelectedProgram(
      formContext,
      podsConfigData.BusinessLogics?.ApplicationInformation
        ?.Evaluation_FieldNamesValues_Map,
      programBusinessLogics.quartech_evaluationchecklistitemstodisplay
    );
  };

  this.showHideFieldsBySelectedProgram = async function (
    formContext,
    fieldNamesValuesMap,
    fieldsValuesToDisplayString
  ) {
    ...
    for (const fieldData of fieldNamesValuesMap) {
      ...
      const fieldVisible = fieldsValuesToDisplay.includes(
        `${fieldData?.Value}`
      );
  ...
  this.customizeTvcCoApplicantForm = async function (formContext) {
    const leadApplicationLookupCtr = formContext.getControl(
      'quartech_leadapplication'
    );
    if (!leadApplicationLookupCtr) return;

    const leadApplicationLookupValue = leadApplicationLookupCtr
      .getAttribute()
      .getValue();
    ...
    const programGuid = await PODS.getSelectedProgramOrStreamGuid(formContext);
    if (!programGuid) {
      const programLookup = await PODS.convertToLookup(
        TVC_PROGRAM_RECORD_GUID,
        'Traceability Value Chain Program',
        PROGRAM_ENTITY_LOGICAL_NAME
      ); // To-do: Nice to have - Get program name from PODS instead of hard coded it here.
      formContext.getAttribute('quartech_program').setValue(programLookup);

      PODS.customizeFormByProgram(formContext);
    }

    const partOfApplicationBundleCtr = formContext.getControl(
      'quartech_partofapplicationbundle'
    );

    const partOfApplicationBundleAttribute =
      partOfApplicationBundleCtr.getAttribute();
    ...
    if (selectedBundleValue !== PART_OF_APP_BUNDLE_VALUE_YES_CO_APPLICATION) {
      partOfApplicationBundleAttribute.setValue(
        PART_OF_APP_BUNDLE_VALUE_YES_CO_APPLICATION
      );
    }
    partOfApplicationBundleCtr.setDisabled(true);

    PODS.populateCoApplicationWithLeadApplication(
      formContext,
      leadApplicationGuid
    );
  ...
  this.populateCoApplicationWithLeadApplication = async function (
    formContext,
    leadApplicationGuid
  ) {
    ...
    if (leadAppData?._quartech_applicant_value) {
      const applicantLookup = await PODS.convertToLookup(
        leadAppData?._quartech_applicant_value,
        leadAppData[
          '_quartech_applicant_value@OData.Community.Display.V1.FormattedValue'
        ],
        'contact'
      );
    ...
    if (leadAppData?.quartech_bundleapplicationid) {
      formContext
        .getAttribute('quartech_bundleapplicationid')
        .setValue(leadAppData?.quartech_bundleapplicationid);
    }
    if (leadAppData?.quartech_submittedon) {
      formContext
        .getAttribute('quartech_submittedon')
        .setValue(new Date(leadAppData?.quartech_submittedon));
    }

    // Applicant Contact Info
    if (leadAppData?.quartech_legalnamefirst) {
      formContext
        .getAttribute('quartech_legalnamefirst')
        .setValue(leadAppData?.quartech_legalnamefirst);
    }
    if (leadAppData?.quartech_legalnamelast) {
      formContext
        .getAttribute('quartech_legalnamelast')
        .setValue(leadAppData?.quartech_legalnamelast);
    }
    if (leadAppData?.quartech_positionorjobtitle) {
      formContext
        .getAttribute('quartech_positionorjobtitle')
        .setValue(leadAppData?.quartech_positionorjobtitle);
    }
    if (leadAppData?.quartech_city) {
      formContext
        .getAttribute('quartech_city')
        .setValue(leadAppData?.quartech_city);
    }
    if (leadAppData?.quartech_telephone) {
      formContext
        .getAttribute('quartech_telephone')
        .setValue(leadAppData?.quartech_telephone);
    }
    if (leadAppData?.quartech_email) {
      formContext
        .getAttribute('quartech_email')
        .setValue(leadAppData?.quartech_email);
    }

    if (leadAppData?._quartech_programintake_value) {
      const intakeLookup = await PODS.convertToLookup(
        leadAppData?._quartech_programintake_value,
        leadAppData[
          '_quartech_programintake_value@OData.Community.Display.V1.FormattedValue'
        ],
        'quartech_programintake'
      );
  ...
  this.getApplicationData = async function (applicationGuid) {
    var data = await Xrm.WebApi.retrieveRecord(
      'msgov_businessgrantapplication',
      applicationGuid,
      `?$select=_quartech_applicant_value, quartech_bundleapplicationid, quartech_submittedon, _quartech_programintake_value, quartech_legalnamefirst, quartech_legalnamelast, quartech_positionorjobtitle, quartech_city, quartech_telephone, quartech_email, _quartech_programintake_value`
    );
  ...
  this.customizeTvcPartOfAppBundle = async function (formContext) {
    ...
    partOfApplicationBundleCtr
      .getAttribute()
      .addOnChange(PODS.handleTvcPartOfApplicationBundle_OnChange);
  };

  this.handleTvcPartOfApplicationBundle_OnChange = async function (
    executionContext
  ) {
    const formContext = executionContext.getFormContext();
    PODS.displayTvcByPartOfAppBundle(formContext);
  ...
  this.displayTvcByPartOfAppBundle = async function (formContext) {
    const selectedBundleValue = formContext
      .getControl('quartech_partofapplicationbundle')
      ?.getAttribute()
      ?.getValue();
    ...
    switch (selectedBundleValue) {
      ...
      default:
        formContext
          .getControl('quartech_bundleapplicationid')
          .setVisible(false);
        formContext.ui.tabs.get('Co-Applications')?.setVisible(false);
        break;
    }
  ...
  this.getSelectedProgramOrStreamGuid = async function (formContext) {
    const programLookupValue = formContext
      .getControl('quartech_program')
      ?.getAttribute()
      ?.getValue();
  ...
  this.initDocumentsTabClickedHandler = async function (formContext) {
    const projectLookupCtrAtt = formContext
      .getControl('quartech_project')
      .getAttribute();
    ...
    documentsTab.addTabStateChange(function () {
      try {
        PODS.documentsTabClicked(formContext, projectGuid);
      } catch (err) {
        console.error(err);

        var documentsTabScriptsNotConfigured =
          error?.message === 'PODS.documentsTabClicked is not a function';
        if (documentsTabScriptsNotConfigured) {
          formContext.ui.setFormNotification(
            'Please contact the PODS development team to configure the form and JS code to handle Documents tab <documents-management-script.js>',
            'ERROR'
          );
        }
      }
